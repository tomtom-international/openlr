...
/**
 * The encoder for the rectangle location type.
 * 
 * <p>
...
public class RectangleEncoder extends AbstractEncoder {
	...
	public final RawLocationReference doEncoding(final Location location,
		...
		LocationCheck locCheck = new RectangleLocationCheck();
		...
		try {
			rawLocRef = new RawRectangleLocRef(location.getID(),
					new GeoCoordinatesImpl(location.getLowerLeftPoint()
							.getLongitudeDeg(), location.getLowerLeftPoint()
							.getLatitudeDeg()), new GeoCoordinatesImpl(location
							.getUpperRightPoint().getLongitudeDeg(), location
							.getUpperRightPoint().getLatitudeDeg()));
